...
# Фикс для Tcl/Tk
def fix_tcl_path():
    """Добавляет пути к Tcl/Tk в переменные окружения"""
    ...
    possible_paths = [
        ...
        r'C:\Users\Anna\AppData\Local\Programs\Python\Python313\tcl\tcl8.6',
    ]

    for path in possible_paths:
        if os.path.exists(path):
            os.environ['TCL_LIBRARY'] = path
            print(f"Установлен TCL_LIBRARY: {path}")
            break

    # Альтернативный путь для TK
    possible_tk_paths = [
        ...
        r'C:\Users\Anna\AppData\Local\Programs\Python\Python313\tcl\tk8.6',
    ]

    for path in possible_tk_paths:
        if os.path.exists(path):
            os.environ['TK_LIBRARY'] = path
            print(f"Установлен TK_LIBRARY: {path}")
            break
